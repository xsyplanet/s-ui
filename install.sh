...
install_s-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/xsyplanet/s-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        last_version=$1
        url="https://github.com/xsyplanet/s-ui/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz"
